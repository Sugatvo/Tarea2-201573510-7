...
def threaded(channel):
    ...
    channel.queue_bind(exchange='intermediario', queue=queue_name, routing_key="ip2")
...
def run():
    ...
    channel.basic_publish(
        exchange='intermediario', routing_key="server", body="\\subscribe-ip2")
    ...
    while(True):
        ...
        if(r_input == "\\clientes" or r_input == "\\mensajes"):
            channel.basic_publish(
                exchange='intermediario', routing_key="server", body=r_input + "-ip2")
            ...
            channel.basic_publish(
                exchange='intermediario', routing_key="server", body="\\salir-ip2")
            print("Bye")
            connection.close()
            break
